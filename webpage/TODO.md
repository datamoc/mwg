...
- **Design choices** (`design/index.html`): why mwg looks the way it does -
  ...
  other, per-example diagrams - the twenty that exist - are
  embedded on their own example's `view.html` page instead, via
  `examples-data.js`'s own `diagram` field. The original eleven
  (`01`-`11_*.svg`) were hand-authored; the other nine
  (`12`-`20_*.svg`) are generated by
  ...
  hand - run it directly to add the next diagram or regenerate any of
  the nine.
...
- [x] The Battle for Wesnoth was added as a 6th reference (README.md,
      ...
      moved out of the numbered roadmap into AGENTS.md's standing policy)
...
- [ ] Each per-example diagram (the twenty that exist, `01`-`20_*.svg`) labels a
      ...
      pass (20 diagrams x several boxes each) rather than risk a rushed,
