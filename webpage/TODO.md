...
- **Documentation**: the published page is `REFERENCE.md` rendered into site chrome
  (module guide + sidebar TOC), with TypeDoc's exhaustive symbol pages under
  `documentation/api/`. Generated by `npm run webpage:docs`, reskinned to match the
...
normally. `webpage/documentation/index.html` is generated from `REFERENCE.md`
(tracked at the repo root); `webpage/documentation/api/` is TypeDoc output.
See "Why TypeDoc needs its own TypeScript" below before touching the API half.
