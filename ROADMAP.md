...
285. [Medium] Fixed-angle map rotation. Turn the view in quarter turns (0/90/180/270), which is what a
     ...
     stays axis-aligned at these angles, so tile choice, fog and pathfinding are untouched, and what
     has to be built is the layer between them and the screen: `Camera.toScreen`/`toWorld` must invert
     the quarter turn so a click lands on the cell the player aimed at, `TiledMap`'s culling must use
     the rotated view rather than an axis-aligned rectangle, and anything drawn *into* the world that
     must stay upright - labels, health bars, damage numbers, the selection cursor's text - needs a
     counter-rotation, since a 180 degree turn would otherwise put every label upside down. A quarter
     turn is exact, so it belongs in the same place the projection options landed (259, 284) rather
     than in the renderer.
286. [Medium] Free map rotation. Any angle, including smooth animation between angles. Beyond 285's
     inverse mapping, this is where the real costs are, and each is a decision rather than a detail:
     adjacent rotated tiles show hairline seams unless they are drawn as one mesh or with a bleed
     margin, so a per-tile rotation is not the same thing as a rotated layer; picking needs the
