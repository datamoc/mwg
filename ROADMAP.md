...
373. [High] A memory bound for the Lua host. The instruction limit does not bound allocation:
    ...
    replacements for `string.rep`/`table.concat` and any other single-instruction allocator,
    an allocation estimate checked from the existing instruction hook, a frozen string
    metatable (today `getmetatable("")` lets one script rewrite `string` methods for the next
    script on the same host), and optionally a Worker-hosted variant that can be terminated
    outright.
374. [Medium] A Content-Security-Policy generated by `tools/emit-page.mjs`: hashes of the inline
    scripts it already writes, `img-src data:`, and `connect-src 'none'` unless the build
    declares its endpoints. Blocks exfiltration by a compromised dependency with nothing for the
    developer to write. Needs a check that the policy holds under `file://` in Chromium, Firefox
    and the Capacitor/desktop hosts before it is on by default.
