...
| `dungeon` | an SPD-shaped mockup: generated floors, three-state fog of war, bump-to-attack, monsters with their own wander/hunt/flee AI (each judges the hero by its own sight, not the hero's), stairs down, plus `mwg/actors` wired in — a `StatBlock` (attack/defense/max HP derived from strength/armor/vitality), items on the floor, and an inventory screen (`Tab`) where equipping a weapon or armor applies its modifiers. Autosaves on every descend and offers to continue on reload, via `mwg/core`'s `SaveSystem` — permadeath: the save is deleted the moment the hero dies, so there is nothing to continue. Arrow keys or the numpad to move, `.` to descend |
