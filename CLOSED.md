...
The shipped, numbered build history for `mwg`, split out of [ROADMAP.md](ROADMAP.md) to
keep that file to what is actually open. Every item below is done; numbers are never
reassigned once given, so this is an append-only record of the order things actually
shipped in, not a priority list. See `ROADMAP.md` for open work, the parked-decisions
list, and the 1.0 exit checklist.
...
349. ~~[Low] Build output: a genuinely single-file `file://` artifact, JS compression, and a
     build-options matrix. Several related build-pipeline ideas: (a) a true standalone HTML
     build with no separate `.js` file at all, inlining the classic IIFE that `emit-page.mjs`
     currently references as script content directly into the page rather than a sibling file;
     (b) compressing the built `mw_games.global.js` (and/or the inlined asset data URIs), with
     compression level as a build option and evaluating a small pure-JS algorithm such as
     `js-lzma` (reported to be only a few kB) against the existing gzip/deflate-shaped
     approaches, weighed against decompression cost at load time; (c) async loading of the
     inlined assets with a splash/loading screen shown while they decode, instead of blocking
     on synchronous inline `<script>` execution; (d) exposing these as a documented matrix of
     build options (single-file vs. multi-file, compressed vs. not, with/without splash
     screen) rather than one fixed pipeline. Must preserve the `file://`-with-no-server
     constraint throughout; a candidate compression library needs to actually work from a
     `file://` script tag, not assume `fetch`/module loading.~~ Landed as
     `tools/single-file.mjs` (`buildSingleFile`) plus `emit-page --single-file
     [--single-file-compress[=level]]`: every `<script src>` an example build loads is
     inlined into one additional `standalone.html`, additive to the existing multi-file
     `index.html` so the build-options matrix is a per-build choice, not a fork of the
     pipeline. Compression uses the browser's own `DecompressionStream('gzip')` rather than a
     shipped decoder like `js-lzma` - it adds zero bytes to the page and reuses the same gzip
     `compress-dist.mjs` already writes for the server-negotiated path, at the cost of needing
     a browser new enough to have `DecompressionStream` (the bootstrap throws a clear error
     otherwise, rather than failing silently). Decoding is async either way, so a minimal
     splash screen (`splash: true` by default, no asset of its own) covers the page until every
     script has run; `splash: false` drops it. Verified in a real browser from `file://`: both
     the compressed (1374.2 KB raw -> 926.6 KB embedded) and uncompressed single-file builds of
     `examples/interface` render correctly with no server, assets decode before the game bundle
     needs them, and the splash element is removed once loading finishes. Fixed a real bug found
     in that verification pass: each example's own "not built" dev-template detection script
     checks `window.__MWG_ASSETS__` synchronously at `DOMContentLoaded`, which only works for a
     *deferred* `<script src>` finishing before that event - the async bootstrap now seeds that
     object (empty, `||`-guarded, same as the compiled asset script's own creation) before
...
374. ~~[Medium] A Content-Security-Policy generated by `tools/emit-page.mjs`.~~ Landed as
    `tools/csp.mjs` (`contentSecurityPolicy`, `withContentSecurityPolicy`, shipped as
    `@datamoc/mw_games/tools/csp`), applied by `emitPage` to `index.html` and to
    `standalone.html`, on by default, with `csp: { connect, eval }` or `csp: false`
    (`--connect=`/`--no-csp` on the command). Every inline script is allowed by its SHA-256
    hash; a compressed standalone page inserts its unpacked scripts inline, so those are hashed
    from the files the multi-file page loads. Two findings changed the proposal. Pixi 8 throws
    at startup under a policy without `'unsafe-eval'` unless `pixi.js/unsafe-eval` is imported,
    which replaces its generated uniform uploads with slower polyfills. So `'unsafe-eval'` stays
    by default: mwg evaluates no data, and a script already in the bundle gains nothing from it.
    And Pixi fetches `data:` textures, so `data:` is a connect source. Verified in Chromium from
    `file://`: `fetch`, an image beacon, a remote `<script src>` and an injected inline script
    are all refused with a `securitypolicyviolation`; all 22 examples and both standalone
    variants start with no console error. The desktop host (WebView2) and Android WebView run
