# Project statistics

Generated for **mwg 0.7.6** on 2026-09-11.

| Area | Statistics |
| --- | ---: |
| Source | 247 TypeScript files, 34,903 lines, 15 modules |
| Tests | 175 files, 1,591 test cases, 21,838 lines |
| Tools | 35 files, 6,335 lines |
| Examples | 22 runnable examples |
| Roadmap | 256/282 items complete, 26 open |
| API | 0 declaration files |
| Bundle | 918.3 kB raw, 273.0 kB gzip |
| Published dist | 2642.6 kB excluding source maps |
| Test-to-source ratio | 0.6x by line count |

## Source modules

| Module | Files | Lines |
| --- | ---: | ---: |
| two-d | 61 | 10,352 |
| core | 41 | 4,654 |
| mwl | 20 | 4,084 |
| roguelike | 22 | 3,167 |
| board | 8 | 2,683 |
| actors | 24 | 2,399 |
| i18n | 9 | 1,630 |
| rpg | 11 | 1,609 |
| ai | 4 | 856 |
| audio | 8 | 793 |
| three-d | 9 | 756 |
| battle | 10 | 555 |
| assets | 6 | 492 |
| simulation | 6 | 443 |
| world | 6 | 394 |

The report is generated by `npm run stats:write` and checked by `npm run stats:check`.
