# Project statistics

Generated for **mwg 0.7.7** on 2026-09-11.

| Area | Statistics |
| --- | ---: |
| Source | 278 TypeScript files, 43,106 lines, 15 modules |
| Tests | 219 files, 2,051 test cases, 28,322 lines |
| Tools | 35 files, 6,346 lines |
| Examples | 22 runnable examples |
| Roadmap | 296/296 items complete, 0 open |
| API | 277 declaration files |
| Bundle | 987.3 kB raw, 293.0 kB gzip |
| Published dist | 3132.3 kB excluding source maps |
| Test-to-source ratio | 0.7x by line count |

## Source modules

| Module | Files | Lines |
| --- | ---: | ---: |
| two-d | 79 | 14,599 |
| mwl | 21 | 5,194 |
| core | 42 | 5,179 |
| roguelike | 22 | 3,429 |
| board | 8 | 2,710 |
| actors | 24 | 2,453 |
| i18n | 10 | 1,835 |
| rpg | 11 | 1,609 |
| ai | 5 | 1,329 |
| battle | 15 | 1,156 |
| audio | 9 | 987 |
| three-d | 9 | 756 |
| simulation | 8 | 699 |
| world | 7 | 603 |
| assets | 6 | 532 |

The report is generated by `npm run stats:write` and checked by `npm run stats:check`.
