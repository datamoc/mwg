# Project statistics

Generated for **mwg 0.7.6** on 2026-09-11.

| Area | Statistics |
| --- | ---: |
| Source | 248 TypeScript files, 35,525 lines, 15 modules |
| Tests | 179 files, 1,633 test cases, 22,454 lines |
| Tools | 35 files, 6,335 lines |
| Examples | 22 runnable examples |
| Roadmap | 262/286 items complete, 24 open |
| API | 0 declaration files |
| Bundle | 919.5 kB raw, 273.4 kB gzip |
| Published dist | 2677.2 kB excluding source maps |
| Test-to-source ratio | 0.6x by line count |

## Source modules

| Module | Files | Lines |
| --- | ---: | ---: |
| two-d | 61 | 10,400 |
| core | 41 | 4,791 |
| mwl | 20 | 4,154 |
| roguelike | 22 | 3,340 |
| board | 8 | 2,683 |
| actors | 24 | 2,399 |
| i18n | 9 | 1,630 |
| rpg | 11 | 1,609 |
| audio | 9 | 987 |
| ai | 4 | 856 |
| three-d | 9 | 756 |
| battle | 10 | 555 |
| assets | 6 | 492 |
| simulation | 6 | 443 |
| world | 6 | 394 |

The report is generated by `npm run stats:write` and checked by `npm run stats:check`.
