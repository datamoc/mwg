/**
 * The Twee-notation parsing engine behind both `two-d/stage.importTwee` and
 * `rpg.importTwee` - the passage/link parsing lives here once, in `core` (already the home
 * of `parseCSV`, another generic text-format parser other modules build their own typed
 * result around), so a bug fix or a format extension lands in one place instead of two
 * silently-drifting copies. `TweeCommand`/`TweeChoice` are structurally compatible with both
 * `two-d/stage`'s `StoryScript`/`StageChoice` and `rpg`'s `EventStoryScript`/`EventChoice` -
 * each module's own `importTwee` wrapper only names its own return type, with no per-field
 * conversion needed.
 *
 * Only structure is imported: each passage's text lines become `say` commands and its
 * `[[links]]` become one closing `ask` whose choices jump (`[[text]]`, `[[text->target]]`
 * and `[[target<-text]]`) - the last text line doubles as the question, since a choice box
 * needs a prompt and repeating it works. No story-format macro language (Harlowe, SugarCube,
 * Chapbook) is interpreted - body text reaches the game untouched, and a link carrying a
 * setter throws instead of silently dropping the behaviour the setter described.
 *
 * ```twee
 * :: Start
 ...
 * @example
 * ```ts
 * import { parseTwee, type TweeStory } from '@datamoc/mw_games/core';
 *
 * const story: TweeStory = parseTwee(`:: Start
 * Pick a door.
 *
 * [[Left->A]]
 * [[Right->B]]
...
export type TweeCommand = { say: string } | { ask: string; choices: TweeChoice[] };
...
export function parseTwee(source: string): TweeStory {
	...
	const story: Record<string, TweeCommand[]> = {};
...
function passageCommands(passage: string, body: string): TweeCommand[] {
	const lines: string[] = [];
	const choices: TweeChoice[] = [];
 ...
 */
function extractLinks(line: string, choices: TweeChoice[], passage: string): string {
	let out = '';
	let rest = line;
	for (;;) {
...
function parseLink(inner: string, passage: string): TweeChoice {
