...
function serializeTarget(targetPath, catalog) {
	if (targetPath.toLowerCase().endsWith('.ftl')) {
		const lines = [
			`# generated by mwg/tools/i18n-edit, do not edit by hand unless flattened plurals are acceptable`,
		];
		let flattened = 0;
...
export function expectedPlaceholders(baseText) {
	return tokenizeMessage(baseText)
		...
		.map(
			(part) =>
				`{${part.token}${part.debug ? '=' : ''}${part.conv ? `!${part.conv}` : ''}${part.spec === undefined ? '' : `:${part.spec}`}}`,
		)
		.join(' ');
}
			...
			? [
					{
						...
						args: [
							'-NoProfile',
							'-Command',
							`(New-Object Media.SoundPlayer '${cuePath.replace(/'/g, "''")}').PlaySync()`,
						],
					},
				]
			: [];
...
export async function main(argv = process.argv.slice(2)) {
	...
	let session = targetFile
		...
		: createEditSession(baseFile.catalog, {
				locale: guessLocale(targetPath),
				direction: guessDirection(guessLocale(targetPath)),
				messages: {},
			});
...
function createEditor({ session, basePath, targetPath, targetIsNew, assetsDir }) {
	...
	async function save() {
		...
		status =
			flattened > 0
				? `saved ${targetPath} (warning: ${flattened} select/plural flattened)`
				: `saved ${targetPath}`;
	}
	...
	function previewCue(displayName, absolutePath) {
		...
		playFirstAvailable(playerCandidates(absolutePath)).then((error) => {
			if (quit) return;
			status = error
				? `could not play: ${String(error?.message ?? error).split('\n')[0]}`
				: `played ${displayName}`;
			draw();
		});
	}
					...
					switch (key.name) {
						...
						case 's':
							if (selected) {
								...
								const answer = (
									await prompt(
										`cue path for [${selected.key}] -> ${audioKey} (empty clears)`,
										own || selected.sound || '',
									)
								).trim();
								if (answer !== '' || own !== '') {
									...
									status =
										answer === ''
											? `cleared cue ${audioKey}`
											: `cue ${audioKey} = ${answer}${checkCueFile(answer)}`;
								}
							}
							break;
						case 'p':
