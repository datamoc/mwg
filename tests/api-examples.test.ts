 ...
 *
 * `two-d`, `render`, `ui` and `board` are not in `MODULES` yet - their `@example` pass
 * hasn't been done. Leaving them out here (rather than committing a failing coverage test)
 * is a visible, temporary gap tracked to be closed, not a silent one: adding them back to
 * this map is exactly the signal that pass is finished.
 */
